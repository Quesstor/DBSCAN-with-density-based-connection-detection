...
def clusterColor(i, norm, permutation, autumn=False):
    ...
    if autumn: return cm.autumn(norm(permutation[i]))
    return cm.rainbow(norm(permutation[i]))


def plot(points, labels, writeIndex=False, title="", legend=False, eps=0, text=[], showAxis=True, autumn=False, randomcolor=False, dotSize=20):
    global currentid, fig, plotcounter
    ax = fig.add_subplot(111)
    dataid = currentid+"."+str(plotcounter)
    plotcounter += 1
    try: os.stat("plots/")
    except: os.mkdir("plots/") 
    ...
    if randomcolor: permutation = np.random.permutation(int(labelsCount)+1)

    getColorFnc = partial(clusterColor, permutation=permutation, norm=norm, autumn=autumn)
    colors = pool.getThreadPool().map(getColorFnc, labels)
    xs, ys = zip(*points)
    ax.scatter(xs, ys, color=colors, s=[dotSize]*len(xs))
    if eps > 0:
        ax = plt.gca()
        for p in points:
