...
test0 = np.concatenate([generatePoints(1, 1), generatePoints(3, 1), [
                       (x, 1) for x in np.arange(1, 3, 0.1)]])
...
# Non linear chains
test2 = np.concatenate([generatePoints(1.5, 3), [(x, pow(x, 3))
...
problem2 = np.concatenate([generatePoints(1, 1, count=100), generatePoints(
    3, 3, count=100), [(x, x) for x in np.arange(1.2, 2.8, 0.03)]])
...
test = np.concatenate([problem1])

#np.save("data", test)
test = np.load("example.npy")
labels = trimming.Algorithm(test, eps=.6, minPts=3, debug=True)
